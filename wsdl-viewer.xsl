...
<xsl:template name="about.detail">
  ...
  <div>
	  Service designed at <a href="http://quantiguous.com">Quantiguous Solutions Pvt. Ltd.</a>
    <br/>
    <br/>    
    <br/>
...
<xsl:attribute name="style">display: none;</xsl:attribute>
        ...
        <xsl:if test="$ENABLE-LINK and $ENABLE-SRC-CODE-PARAGRAPH">
        <div class="label">Source code:</div>
        <div class="value">
<xsl:call-template name="render.source-code-link"/>
</div>
...
<!-- outputs the individual elements in the operations -->
...
-->
	<li>
		...
		<!-- the type name is either from the element from the referenced element -->
    <xsl:variable name="type-name">
      <xsl:choose>
        <xsl:when test="@ref">
          <xsl:call-template name="xslt.substring-after-if-contains">
            <xsl:with-param name="arg" select="$consolidated-xsd[@name = $elem-name]/@type"/>
    ...
    <!-- while this is strictly not needed for an analyst, being explicit about references is a good thing -->
    <xsl:if test="@ref">
      <small style="color:red"> ref </small>
    </xsl:if>
    
    <!-- render the type information -->
    <xsl:call-template name="render-type">
			 <xsl:with-param name="type-local-name" select="$type-name"/>
    </xsl:call-template>
    
    <!-- append the referenced elements documentation -->
    <xsl-if test="@ref">
    	<div style="color:green">
        <xsl:value-of select="$consolidated-xsd[@name = $elem-name]/xsd:annotation/xsd:documentation" disable-output-escaping="yes"/>
      </div>        
    </xsl-if>
...
<!-- returns the unqualified (local) type or reference name, returns undefined if there's no type -->
<xsl:template name="xsd.element-type">
  <!-- figure out if the element is a reference or has a type defined -->
  ...
  <!-- namespaces are ignored, so we pick up only the local name-->
	<xsl:variable name="type-local-name" select="substring-after($ref-or-type, ':')"/>
  
  <!-- we print the type/ref name a local name if there was a namespace, or just the type name -->
	<xsl:variable name="type-name">
		<xsl:choose>
			<xsl:when test="$type-local-name">
        <xsl:value-of select="$type-local-name"/>
...
<div id="footer">
	All Rights Reserved. This API Taxanomy has been designed by Quantiguous Solutions Pvt. Ltd.
</div>
</xsl:template>
...
<xsl:param name="version"/>
<div class="page">
	<a class="target" name="page.about"><h2>Notice</h2></a>
...
<xsl:template name="xslt.substring-after-if-contains">
  <xsl:param name="arg"/>
  <xsl:param name="delim"/>
  
  <xsl:variable name="arg-after" select="substring-after($arg, $delim)"/>
  <xsl:variable name="result">
    <xsl:choose>
      <xsl:when test="$arg-after">
        <xsl:value-of select="$arg-after"/>
      </xsl:when>
      <xsl:otherwise>
